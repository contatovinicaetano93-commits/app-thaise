  ...
  return (
    <div>
      <PageFeedHeader
        ...
        subtitle="Rascunhos gerados automaticamente às sextas — revise e envie ao cliente"
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title="Nenhum relatório 360 ainda"
          description="Os rascunhos são gerados automaticamente toda sexta-feira para empreendimentos ativos."
        />
      ) : (
        <div className="space-y-6">
          {drafts.length > 0 && <ReportSection title="Rascunhos para revisão" reports={drafts} actionId={actionId} onApprove={id => updateStatus(id, 'approved')} onSend={id => updateStatus(id, 'sent')} />}
          {approved.length > 0 && <ReportSection title="Aprovados — prontos para envio" reports={approved} actionId={actionId} onSend={id => updateStatus(id, 'sent')} />}
      ...
      <div className="space-y-3">
        {reports.map(r => (
          <div key={r.id} className="bg-white border border-gray-200 rounded-xl p-4">
            ...
            {r.risks && (
              <p className="text-sm text-amber-700 bg-amber-50 rounded-lg px-3 py-2 mt-2">🛡️ {r.risks}</p>
            )}
